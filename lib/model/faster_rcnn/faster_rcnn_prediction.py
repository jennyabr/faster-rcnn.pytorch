...
def faster_rcnn_prediction(data_manager, model, cfg):
    ...
    model.eval()
    all_boxes = np.zeros(num_classes, num_images)
    empty_prediction = np.transpose(np.array([[], [], [], [], []]), (1, 0))
    for i in range(num_images):
        ...
        def transform_preds_to_img_coords():
            ...
            return preds_clipped_to_img_size
        bbox_coords = transform_preds_to_img_coords()

        im_scaling_factor = im_info[0][2]
        bbox_coords /= im_scaling_factor

        ############################################
        scores = scores.squeeze()
        bbox_coords = bbox_coords.squeeze()
        pred_end = time.time()
        detect_time = pred_end - pred_start
        misc_tic = time.time()
